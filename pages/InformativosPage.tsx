...
interface InformativosPageProps {
...
const InformativosPage: React.FC<InformativosPageProps> = ({ onNavigate }) => {

    const actionCards = [
        { id: 'visualizar', icon: EyeIcon, title: 'Visualizar' },
        { id: 'pesquisar', icon: SearchIcon, title: 'Pesquisar' },
        { id: 'adicionar', icon: PlusIcon, title: 'Adicionar' },
    ...
    <section className="space-y-6 pb-8">
          ...
          <ol className="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse text-sm">
            ...
            <li aria-current="page">
              <div className="flex items-center">
                ...
                <span className="ms-1 font-medium text-slate-400 dark:text-slate-500 md:ms-2">Informativos</span>
              </div>
            </li>
          </ol>
        </nav>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4 sm:gap-6">
        {actionCards.map(card => (
            <ActionCard key={card.id} icon={card.icon} title={card.title} />
        ))}
      </div>
